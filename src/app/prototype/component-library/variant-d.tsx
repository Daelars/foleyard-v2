// PROTOTYPE ONLY — variant D, "Mockup match". Specimen grid composed to the
// supplied reference mockup: bento cards on a near-black canvas with faint red
// ambient blooms, red-lit acrylic primary, charcoal secondaries, inset fields,
// status badges, dark tags with a red AI score, file row, command panel,
// progress, alerts, pagination, and tooltip. Controls come from
// ./variant-d-kit so A/B/C stay untouched.
"use client";
...
const COMMAND_ROWS = [
  ...
  { label: "Analyze untagged files", hint: "view" },
];

export function VariantD({ settings, onToggleSetting }: SpecimenState) {
  ...
  const filtered = COMMAND_ROWS.filter((row) =>
    ...
    <DSurface className="relative min-h-full overflow-x-clip bg-[#0a0a0e]">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-4 px-6 py-8">
        <header className="flex flex-wrap items-start justify-between gap-4 pb-2">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Core components matching the app&rsquo;s visual style. Glassy, minimal, and
              functional.
            </p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <DCard title="Toggles & Switches" sub="Switch, checkbox, and radio inputs." className="col-span-12 md:col-span-6 lg:col-span-4">
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <div className="grid content-start gap-3">
                <DRadio name="d-filter" label="All files" checked={filter === "all"} onChange={() => setFilter("all")} />
                <DRadio name="d-filter" label="Untagged only" checked={filter === "untagged"} onChange={() => setFilter("untagged")} />
                <DRadio name="d-filter" label="Custom filter" checked={filter === "custom"} onChange={() => setFilter("custom")} />
              </div>
            </div>
          </DCard>
          ...
          <DCard title="Dropdown / Command" sub="Command palette or menu." className="col-span-12 lg:col-span-4">
            <DCommandPanel>
              ...
              <div className="grid gap-0.5 p-1.5">
                {(filtered.length > 0 ? filtered : COMMAND_ROWS).slice(0, 4).map((row, index) => (
                  <DCommandRow
                    key={row.label}
                    active={filtered.length > 0 ? index === activeIndex : index === 0}
                    hint="view"
            ...
            <DCommandPanel>
              <div className="flex items-center gap-2.5 border-b border-white/[0.07] px-4">
                ...
                <input
                  ...
                  placeholder="Type a command or sound…"
                  aria-label="Type a command or sound"
                  aria-controls="command-palette-results-d"
                  aria-activedescendant={
                    activeIndex >= 0 ? `command-palette-entry-d-${activeIndex}` : undefined
                  }
              ...
              >
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row, index) => (
                    <DCommandRow
                      key={row.label}
                      id={`command-palette-entry-d-${index}`}
                      role="option"
