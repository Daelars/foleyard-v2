...
import { ExtensionDialogShell } from "@/components/workspace/extensions/ExtensionDialogShell";
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
...
import { Alert, AlertDescription } from "@/components/kit/alert";
import { Switch } from "@/components/kit/switch";
import { Label } from "@/components/kit/label";
import { Badge } from "@/components/kit/badge";
import { Card, CardContent } from "@/components/kit/card";
import { isDesktopApp } from "@/lib/desktop";
import { ExtensionStatusBanner } from "@/components/workspace/extensions/dialog-fields";
...
interface FolderJanitorV2DialogProps {
  open: boolean;
...
/**
 * Folder Janitor v2 dialog (Application context).
 *
 * Renderer-owned orchestration over the generic v2 transport: scans
 * through the immediate scan commands, index removals through the
 * immediate remove-files command, and folder deletes through the
 * prepare/review/apply plan contract. Layout mirrors the retired v1
 * dialog; the engine stays generic and this dialog only sequences
 * its endpoints.
 */
export function FolderJanitorV2Dialog({
  open,
  onOpenChange,
  initialTarget = "library",
  initialFolderPath,
}: FolderJanitorV2DialogProps) {
  const {
    progress,
    isScanning,
    result,
  ...
  } = useFolderJanitorV2({ open, initialTarget, initialFolderPath });
