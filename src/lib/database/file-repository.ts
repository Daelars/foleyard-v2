...
getScanCleanupRows(): Array<{ path: string; libraryRoot: string | null; removedAt: string | null }> { return reads.getScanCleanupRows({ sqlite: this.sqlite, db: this.db }); }
...
batchTouchActiveFiles(paths: string[], now: string): void { return batch.batchTouchActiveFiles({ sqlite: this.sqlite, db: this.db }, paths, now); }
...
analyzeStatistics(): void { this.sqlite.exec("ANALYZE"); }
...
export const getScanCleanupRows = () => getFileRepo().getScanCleanupRows();
...
export const batchTouchActiveFiles = (paths: string[], now: string) => getFileRepo().batchTouchActiveFiles(paths, now);
...
export const analyzeStatistics = () => getFileRepo().analyzeStatistics();
